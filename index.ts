  ...
  const j = sourceFile.transform((traversal) => {
    ...
    switch (node.kind) {
      case ts.SyntaxKind.JsxAttributes:
        ...
        if (!jsxNode.properties.some((e) => ((e.name as Identifier)?.escapedText as string)?.indexOf(':') === 1))
          return node;
        ...
        for (const property of properties) {
          ...
          if (name.indexOf('s:') === 0) {
            addStyle.push([name.replace('s:', ''), (property.initializer as JsxExpression).expression!]);
